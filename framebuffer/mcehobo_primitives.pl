...
#
# With regards to MCE::Shared, locking is not required when involving a
# single operation. However, locking is necessary for $count += 4 because
# involves 2 IPC trips { FETCH and STORE } to the shared-manager process.
#
#   tie my @order, 'MCE::Shared', (...);
#   tie my $count, 'MCE::Shared', 0;
#
#   tied(@order)->lock; @order = ();           tied(@order)->unlock; # okay
#   tied(@order)->lock; $name = shift(@order); tied(@order)->unlock; # okay
#   tied($count)->lock; $count += 4;           tied($count)->unlock; # required
#
# Another possibility is using the OO interface (no TIE overhead).
#
#   my $order = MCE::Shared->array(...);
#   my $count = MCE::Shared->scalar(0);
#
#   $order->clear;           # $order->lock, $order->unlock not required
#   $name = $order->shift;
#   $count->incrby(4);       # ditto, a single IPC operation
#
# Debugging and dumping shared data.
#
#   warn Dumper( tied(@order)->export ), "\n";
#   warn Dumper( $order->export ), "\n";
##
...
sub run_thread {
    ...
    while (scalar(@order)) {
        ...
        # Because @order is shared and cleared in finish, is sufficient.
        # Therefore, $RUNNING isn't shared including not checked here.
        # TBH, I only came to this realization after writing mcechnl_*.
...
=head1 SYNOPSIS
 ...
 perl mcehobo_primitives.pl [--dev=device number] [--x=X emulated resolution] [--y=Y emulated resolution] [--pixel=pixel size] [--noaccel]
