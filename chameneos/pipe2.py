...
#    contributed by Mario Roy 2025-04-08
##
...
class Pipe:
    ...
    def __init__(self):
        self.r, w = os.pipe() # read from pipe via os module, faster
        self.w = os.fdopen(w, 'wb', 0); # fdopen, so can self.w.flush()
    ...
    def recv(self):
        slen = struct.unpack('!i', os.read(self.r, 4))[0]
        if slen > 0: return os.read(self.r, slen).decode('utf-8')
        if slen < 0: return None
        return ""

    def close(self):
        self.w.close() # os.fdopen file handle
        os.close(self.r) # os.pipe file descriptor
...
_chnls = [Pipe() for _ in range(1 + 10)]
