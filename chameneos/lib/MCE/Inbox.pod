...
This document describes MCE::Inbox version 0.001
...
A MCE::Inbox is a shared container for sending and receiving messages via
L<Thread::Queue>, when C<threads> is present, or L<MCE::Shared::Queue>
otherwise. Serialization is provided by L<Sereal> if available or L<Storable>.
Both ends of the C<inbox> for a given ID can be read and written by as many
workers as you want concurrently.
...
Acquire the number of messages pending for given ID or list of IDs.
...
Signal that there is no more work to be sent for given ID. Omitting an ID
will end all IDs. Once ended, no more messages may be sent.
...
MCE::Inbox requires Perl 5.10.1 or later and MCE::Shared 1.834 minimally.

On Microsoft Windows, please use MCE::Shared 1.835 for improved C<dequeue>
performance using MCE::Queue or MCE::Shared->queue. The fix is in MCE::Util
1.834. Loading threads first before this module will use Thread::Queue.
It must be 3.07 or later with C<end> and C<limit> capabilities.
...
Copyright (C) 2018 by Mario E. Roy
